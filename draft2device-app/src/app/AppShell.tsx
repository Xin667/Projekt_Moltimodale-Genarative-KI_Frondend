...
export function AppShell({ children, navigation, projects}: AppShellProps) {
  return (
    <div className="min-h-screen bg-[#FAF8F4] flex flex-col font-sans antialiased text-[#1E2430]">
      ...
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-20 gap-3 p-6 max-w-[1800px] w-full mx-auto">
        
        {/* LINKE SPALTE: DEIN Stepper als Navigation (3 von 12 Spalten) */}
        <aside className="lg:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-[#D9D3C7]/40 flex flex-col">
           {projects} 
        </aside>

        {/* MITTLERE SPALTE: Der Hauptarbeitsbereich für die Schritte (6 von 12 Spalten) */}

        <section className="lg:col-span-12 bg-white p-8 rounded-2xl shadow-md border border-[#D9D3C7]/40 min-h-[700px]">
          {navigation}
          {children}
        </section>

        {/* RECHTE SPALTE: KI-Assistent & Empfehlungen (3 von 12 Spalten) */}
        <aside className="lg:col-span-4 bg-white p-6 rounded-2xl shadow-sm border border-[#D9D3C7]/40 flex flex-col gap-4">
